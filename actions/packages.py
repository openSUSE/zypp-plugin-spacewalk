...
def setLocks(package_list, cache_only=None):
    """We must locks the provided packages"""
    if cache_only:
    ...
    log.log_debug("Called packages.setLocks", package_list)
    # zypper support for handling locks is insuffcient, so we
    # write a complete new locks file here.
    tmpfile = NamedTemporaryFile(dir='/etc/zypp', prefix='locks', delete=False)

    tmpf = open(tmpfile.name, 'w')
    tmpf.write('# Generated by zypp-plugin-spacewalk:packages.setLocks action\n')
    tmpf.write('# Do not edit! Changes will be lost!\n')
    for pkg in package_list:
	# pkg is a tuple (n, v, r, e, a)
	tmpf.write('\n')
	tmpf.write('type: package\n')
	tmpf.write('match_type: exact\n')
	tmpf.write('case_sensitive: on\n')
	tmpf.write('solvable_name: %s\n' % pkg[0])
    tmpf.close()

    os.rename(tmpfile.name, '/etc/zypp/locks')
    return (0, "Wrote /etc/zypp/locks", {})
