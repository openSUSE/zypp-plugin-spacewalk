...
from zypper import lock_helper
...
def setLocks(package_list, cache_only=None):
    ...
    official_lock_file = '/etc/zypp/locks'
    locks = read_lock_file(official_lock_file)

    for pkg in package_list:
        # pkg is a tuple (n, v, r, e, a)
        lock_solvable(locks, pkg[0])

    tmpfile = NamedTemporaryFile(dir='/etc/zypp', prefix='locks', delete=False)
    write_lock_file(locks, tmpfile.name)
    os.rename(tmpfile.name, official_lock_file)

    return (0, "Wrote %s" % official_lock_file, {})
